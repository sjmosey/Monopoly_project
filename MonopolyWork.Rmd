...
p1<- data.frame(turn,position, money, properties)
p2<- data.frame(turn,position, money, properties)
p3<- data.frame(turn,position, money, properties)
p4<- data.frame(turn,position, money, properties)
...
{
  isDone<-function(p) #function checks if player taking turn is out of money or not, parameter is a player data frame
    { 
	  if (p$money <= 0){
	    return()
	  }
...
roll<-function(x,p) #function to roll dice just using sampling twice
    ...
    x<-sample(1:6,2,replace=TRUE)
      if(x[1]==x[2]){
        return(p) #right now if you roll doubles it will just print the position the player was on before rolling doubles
      }
    else{
      spaces<-(x[1]+x[2]) #if you don't roll doubles the dice are summed
      getDesination<-function(p, pos, dicesum) #function that gets the board position
            { 
            	pos<-(pos + dicesum) %% 40 #sum of dice added to current player position for new position
            	p$position<-pos
            	return(p)
      }
  getDesination(p, p$position, spaces)
    }
 }
 roll(d,p)
     #  move<-function(p, pos){
